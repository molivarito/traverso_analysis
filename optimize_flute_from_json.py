...
def optimize_flute_from_json_full(
    ...
    flute_data_instance: Optional[FluteData] = None,
    flute_dir_path_str: Optional[str] = None 
) -> tuple[dict, dict, dict, str, dict, float, dict, dict, list]:
    """
    ...
    if current_flute_data_instance is None:
        ...
        try:
            ...
            if current_flute_data_instance.validation_errors:
                ...
                # Podríamos decidir retornar aquí si los errores son críticos para la optimización
                # return {}, {}, {}, current_flute_data_instance.flute_model, {}, 0.0, {}, {}, []
    ...
    if not found_embouchure_for_optim:
        ...
        return {}, {}, {}, flute_model_name, {}, diapason_a4_hz_gui, {}, {}, []
    logger.info(f"Agujeros procesados desde JSON para '{flute_model_name}'.")
    ...
    logger.info(f"Tabla de digitaciones procesada desde JSON para '{flute_model_name}'.")
    ...
    try:
        ...
        logger.info(f"InstrumentGeometry creada para '{flute_model_name}'")
    ...
    try:
        ...
        return {}, {}, {}, flute_model_name, target_frequencies_hz_map, diapason_a4_hz_gui, {}, {}, []
    ...
    for note_name in notes_to_optimize: 
        ...
        try:
            ...
            optimized_value_m = my_geom.optim_params.get_active_values()[0]
            ...
            if len(f_res_optim) >= 1:
                logger.info(f'  Final deviation: {f_res_optim[0]} vs {target_freq}')
            ...
            if freqs_for_pf_calc:
                ...
                if hasattr(optim_tool, 'x_interp') and optim_tool.x_interp is not None and \
                   ...
                   hasattr(optim_tool, 'frequencies') and optim_tool.frequencies is not None:
                    ...
                    logger.error(f"  Atributos de P/F NO encontrados o son None en optim_tool para {note_name} después de solve().")
                    pressure_flow_data_per_note[note_name] = {} 
            else:
                logger.info(f"  No hay frecuencias válidas para calcular P/F para {note_name}.")
                pressure_flow_data_per_note[note_name] = {}
            
            physics_states_per_note[note_name] = copy.deepcopy(optim_tool.instru_physics) 
            logger.info(f"  Estado de InstrumentPhysics (geometría) guardado para {note_name} después de la optimización.")
            notes_actually_optimized.append(note_name)
    ...
    return optimized_chimney_heights_mm, initial_admittance_data, optimized_admittance_data, flute_model_name, target_frequencies_hz_map, diapason_a4_hz_gui, physics_states_per_note, pressure_flow_data_per_note, notes_actually_optimized
